...
export async function collectConfigValues(
	...
	for (const [key, prop] of Object.entries(
		...
		if (value !== undefined || schemaProp.default !== undefined) {
			// Convert the value to the appropriate type before storing
			configValues[key] = convertValueToType(value ?? schemaProp.default, schemaProp.type)
		}
	}
...
export function chooseConnection(server: RegistryServer): ConnectionDetails {
	...
	const priorityOrder = ["docker", "npx", "uvx"]
